...
class DQN:
    def __init__(self, args):
        ...
        self._optimizer = optim.Adam(self._behavior_net.parameters(), lr=0.0005)
        # memory
    ...
    def select_action(self, state, action_space, epsilon=0.):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if total_steps < args.warmup:
                # wramup: fill the agent’s memory using usually a random policy
                action = action_space.sample()
            else:
                action = agent.select_action(state, action_space, epsilon)
